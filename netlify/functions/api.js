...
async function handleAdminStats(req, blogStore, headers, admin) {
  ...
  try {
    ...
    const stats = {
      ...
      posts: {
        total: totalPublicPosts + totalPrivatePosts,
        public: totalPublicPosts,
        private: totalPrivatePosts
      },
      engagement: {
        totalReplies: totalReplies,
        averageRepliesPerPost: postBlobs.blobs.length > 0 ? (totalReplies / postBlobs.blobs.length).toFixed(2) : 0
      }
    };
...
// Keep all existing handlers (login, feeds, posts, etc.) unchanged...
// [Previous handlers remain the same - login, logout, validateAuth, feeds, posts, etc.]
...
// Keep existing utility functions and other handlers...
...
// Media detection functions (unchanged)
...
// Keep existing feed and post handlers...
...
async function handleDefault(req, headers) {
  const docs = {
    message: "Blog API v4.0 - Admin Features & URL Profile Pictures",
    endpoints: {
      // Authentication
